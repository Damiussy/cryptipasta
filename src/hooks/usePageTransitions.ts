...
export function usePageTransitions() {
  ...
  // NOUVELLE FONCTION pour préserver les balises HTML
  const wrapTextNodesWithSpans = (node: Node, delay: number = 0): number => {
  ...
  const animateText = useCallback(() => {
    ...
    // SEULEMENT les textes COURTS - animation lettre par lettre AVEC préservation HTML
    ...
    shortTextSelectors.forEach((selector, selectorIndex) => {
      ...
      elements.forEach((element, elementIndex) => {
        ...
        if (element.querySelector('.letter-blur-in')) return; // Éviter double animation
    ...
    // ANIMATION POUR LES LONGS TEXTES <p> avec préservation HTML
    ...
    longTextElements.forEach((element, index) => {
      if (!(element instanceof HTMLElement)) return;
      if (!isElementInViewport(element)) return;
      if (element.querySelector('.letter-blur-in, .word-blur-in')) return; // Éviter double animation
      ...
      if (isHomePage || textLength > 300) {
        // Animation par mots pour les longs textes - PRÉSERVATION HTML
        ...
        // Animation lettre par lettre pour les textes moyens - PRÉSERVATION HTML
        const baseDelay = 200 + (index * 50);
        wrapTextNodesWithSpans(element, baseDelay);
      }
    });

    // FOOTER animation avec préservation HTML
